...
import { GrokAgent, type GrokAgentOptions } from "./agent/grok-agent.js";
...
function parseMaxToolRounds(input?: string): number {
...
function getAgentOptions(options: {
  chatOnly?: boolean;
  noTools?: boolean;
  maxToolRounds?: string;
}): GrokAgentOptions {
  const chatOnly = Boolean(options.chatOnly);
  const toolsEnabled = !(chatOnly || options.noTools);
...
async function handleCommitAndPushHeadless(
  ...
  baseURL?: string,
  model?: string,
  agentOptions: GrokAgentOptions = {}
): Promise<void> {
  try {
    const agent = new GrokAgent(apiKey, baseURL, model, agentOptions);
...
program
  ...
  .option(
    "-c, --chat-only",
    "disable all tools and autonomous behavior for a conversational chat session"
  )
  .option(
    "--no-tools",
    "disable tools while keeping lightweight interactive assistance"
  )
  .option(
    "--max-tool-rounds <rounds>",
    "maximum number of tool execution rounds (default: 8)",
    "8"
  )
...
gitCommand
  ...
  .action(async (options) => {
    ...
    try {
      ...
      await handleCommitAndPushHeadless(apiKey, baseURL, model, agentOptions);
    } catch (error: any) {
