...
export class GrokAgent extends EventEmitter {
  ...
  constructor(
    ...
    this.toolsEnabled = options.toolsEnabled ?? !this.chatOnly;
    this.maxToolRounds = Math.max(1, options.maxToolRounds ?? 8);
    ...
    this.messages.push({
      role: "system",
      content: this.buildSystemPrompt(customInstructionsSection),
    });
  }

  private buildSystemPrompt(customInstructionsSection: string): string {
    if (!this.toolsEnabled) {
      const modeInstruction = this.chatOnly
        ? "You are operating in chat-only mode. Answer conversationally and never attempt tool use."
        : "Tool use is disabled for this session. Respond conversationally and do not attempt autonomous tool execution.";

      return `You are Grok CLI, an AI assistant that helps with questions, coding guidance, and system operations.${customInstructionsSection}
...
- Do not mention unavailable tools unless the user explicitly asks
...
- If the request has multiple clear steps, you may create a todo list to plan your approach
...
Current working directory: ${process.cwd()}`;
  ...
  private shouldBypassToolsForMessage(message: string): boolean {
    ...
    if (commonGreetings.has(normalized)) {
      return true;
    }

    return normalized.length <= 14 && /^(hi|hello|hey|yo|sup|thanks|thank you|ok|okay)[!.?]*$/i.test(normalized);
  }

  private buildShortCasualResponse(message: string): string {
    const normalized = message.trim().toLowerCase();
  ...
  getRuntimeConfig() {
    return {
      ...
      model: this.grokClient.getCurrentModel(),
    };
  }
  ...
  async processUserMessage(message: string): Promise<ChatEntry[]> {
    ...
    try {
      if (this.shouldBypassToolsForMessage(message)) {
        const response = this.buildShortCasualResponse(message);
  ...
  async *processUserMessageStream(
    ...
    try {
      if (this.shouldBypassToolsForMessage(message)) {
        yield {
          type: "content",
          content: this.buildShortCasualResponse(message),
        };
        yield { type: "done" };
        return;
      }
