...
# Start in chat-only mode with no tools
grok --chat-only

# Start with tools disabled but keep lightweight interactive assistance
grok --no-tools
...
Options:
  ...
  -c, --chat-only             Disable all tools and autonomous behavior
  --no-tools                  Disable tools while keeping lightweight assistance
  -p, --prompt <prompt>       Process a single prompt and exit
  --max-tool-rounds <n>       Max number of tool execution rounds (default: 8)
```

### How To Prevent Tool Loops

If you want pure conversational behavior, start Grok with:

```bash
grok --chat-only
```

If you still want the normal interactive UI but do not want tool execution, use:

```bash
grok --no-tools
```

If you keep tools enabled, Grok now defaults to a safer limit:

```bash
grok --max-tool-rounds 8
```

Short casual prompts such as `Hi.` now stay conversational and skip autonomous tool calls entirely.
